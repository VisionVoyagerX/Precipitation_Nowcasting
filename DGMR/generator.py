...
class Generator(snt.Module):
    ...
    def __init__(self, lead_time=90):
        super().__init__(name=None)
        # self._batch_size=batch_size
        #self.prec_normalizer = tf.keras.layers.Normalization(axis=None)

        #self.ech_normalizer = tf.keras.layers.Normalization(axis=None)
        #self.mot_normalizer = tf.keras.layers.Normalization(axis=None)

        self.twoStream_3DConvNet = TwoStream_3DConvNet()
        self._cond_stack = ConditioningStack()
        self._sampler = Sampler(lead_time)
    ...
    def __call__(self, inputs, is_training=True):
        ...
        inputs_fused = self.twoStream_3DConvNet(
            inputs, is_training=is_training)
        # inputs = self.prec_normalizer(
        #    inputs[..., :1])
        initial_states = self._cond_stack(
            inputs_fused, is_training=is_training)
    ...
    def call(self, inputs, is_training=True):
        ...
        #inputs_masked = self.mask_layer(inputs)
        # FIXME check if dimensions are correct
        _, _, height, width, _ = (None, None, 768, 704, None)
        initial_states = self._cond_stack(
            inputs, is_training=is_training)
        predictions = self._sampler(
            initial_states, [height, width], is_training=is_training)
...
class TwoStream_3DConvNet(snt.Module):
    def __init__(self,):
        ...
        self.conv3D_prec = layers.SNConv3D(output_channels=24, kernel_size=3)
        self._block_prec = discriminator.DBlock(
            output_channels=24, conv=layers.SNConv3D, downsample=False)

        self.conv3D_echo = layers.SNConv3D(output_channels=24, kernel_size=3)
        self._block_ech = discriminator.DBlock(
            output_channels=24, conv=layers.SNConv3D, downsample=False)

        self.conv3D_mot = layers.SNConv3D(output_channels=24, kernel_size=3)
        self._block_mot = discriminator.DBlock(
            output_channels=24, conv=layers.SNConv3D, downsample=False)

        self.conv3D_fuse = layers.SNConv3D(output_channels=48, kernel_size=3)
        self._block_fuse = discriminator.DBlock(
            output_channels=48, conv=layers.SNConv3D, downsample=False)

    def __call__(self, tensor, is_training=True):
        #tensor_list = tf.unstack(tensor, axis=-1)

        #stream1 = self._bn4(tensor[..., :1], is_training=is_training)
        # stream1 = tf.nn.relu(self.conv3D_prec(
        #    tensor[..., :1], is_training=is_training))
        stream1 = self._block_prec(tensor[..., :1], is_training=is_training)

        #stream2 = tf.nn.relu(self.conv3D_echo(tensor[..., 1:2], is_training=is_training))

        # stream3 = tf.nn.relu(self.conv3D_mot(
        #    tensor[..., 1:3], is_training=is_training))
        stream3 = self._block_mot(tensor[..., 1:3], is_training=is_training)

        # stream = tf.nn.relu(self.conv3D_fuse(
        #    tf.concat([stream1, stream3], axis=-1), is_training=is_training))
        stream = self._block_fuse(
            tf.concat([stream1, stream3], axis=-1), is_training=is_training)
...
class ConditioningStack(snt.Module):
    ...
    def __init__(self):
        super().__init__(name=None)
        #self.linear = layers.Linear(output_size=1)
...
class Sampler(snt.Module):
    ...
    def __init__(self, lead_time=90):
        super().__init__(name=None)
        self._num_predictions = lead_time // 5
        self._latent_stack = latent_stack.LatentCondStack()

        self._conv_gru4 = ConvGRU(num_channels=384)
        self._conv4 = layers.SNConv2D(kernel_size=3, output_channels=768)
        ...
        self._conv3 = layers.SNConv2D(kernel_size=3, output_channels=384)
        ...
        self._conv2 = layers.SNConv2D(kernel_size=3, output_channels=192)
        ...
        self._conv1 = layers.SNConv2D(kernel_size=3, output_channels=96)
    ...
    def __call__(self, initial_states, resolution, is_training=True):
        ...
        # Layer 4 (bottom-most).
        hs, _ = snt.static_unroll(
            self._conv_gru4, tf.stack(hs), init_state_4)
        hs = [self._conv4(h, is_training=is_training) for h in tf.unstack(hs)]
        hs = [self._gblock4(h, is_training=is_training) for h in hs]
        hs = [self._g_up_block4(h, is_training=is_training) for h in hs]

        # Layer 3.
        hs, _ = snt.static_unroll(self._conv_gru3, tf.stack(hs), init_state_3)
        hs = [self._conv3(h, is_training=is_training) for h in tf.unstack(hs)]
        hs = [self._gblock3(h, is_training=is_training) for h in hs]
        hs = [self._g_up_block3(h, is_training=is_training) for h in hs]

        # Layer 2.
        hs, _ = snt.static_unroll(self._conv_gru2, tf.stack(hs), init_state_2)
        hs = [self._conv2(h, is_training=is_training) for h in tf.unstack(hs)]
        hs = [self._gblock2(h, is_training=is_training) for h in hs]
        hs = [self._g_up_block2(h, is_training=is_training) for h in hs]

        # Layer 1 (top-most).
        hs, _ = snt.static_unroll(self._conv_gru1, tf.stack(hs), init_state_1)
        hs = [self._conv1(h, is_training=is_training) for h in tf.unstack(hs)]
