...
def Dataset(tfr_dir,  batch_size, prob=True, shuffle=True):
    files = tf.io.gfile.glob(str(tfr_dir)+'/*.tfrecords')
    files = tf.data.Dataset.from_tensor_slices(files)
    if shuffle:
        files = files.shuffle(buffer_size=len(files)//1)  # default 10
    ...
    '''dataset = dataset.map(fliping,
                          num_parallel_calls=tf.data.AUTOTUNE)'''
    if shuffle:
        dataset = dataset.shuffle(buffer_size=2)  # default 500
...
def slicing(x0, x1, x2, x3):
    ...
    lead_frames = 1  # default 18
    return (x0[..., 0:2], x1[:lead_frames, ...], x2[:num_conditioning_frames + lead_frames, ...], x3)
